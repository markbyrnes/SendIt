...
#sets last seen time before requests
@app.before_request
def before_request(): 
...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        profile_pic = request.files['profile_pic'] #start of profile uploading
        pic_filename = secure_filename(profile_pic.filename) 
        pic_name = str(uuid1()) + "_" + pic_filename #adds unique id to uploaded image
        profile_pic.save(os.path.join(Config.PROFILE_FOLDER, pic_name)) #save image

        current_user.profile_pic = pic_name #sets users profile_pic to above
...
def messages():
    ...
    messages = current_user.messages_received.order_by(
        Message.timestamp.desc()).paginate(
            ...
            error_out=False)
    next_url = url_for('mmessages', page=messages.next_num) \
